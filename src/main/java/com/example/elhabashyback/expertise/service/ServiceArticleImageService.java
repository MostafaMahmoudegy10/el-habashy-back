...
public class ServiceArticleImageService {
    ...
    public ServiceArticleImageResponse upload(MultipartFile file) {
        ...
        try {
            ...
            String publicId = baseFolder + "/services/" + UUID.randomUUID();
            return ServiceArticleImageResponse.from(cloudinaryUploadClient.uploadImage(
                    stagedFile,
                    fileName,
                    publicId,
                    contentType,
    ...
    private void validate(MultipartFile file, String contentType) {
        ...
        if (file.getSize() > MAX_IMAGE_BYTES) {
            throw new BadRequestException("Service article images must not exceed 10 MB");
        }
    }
